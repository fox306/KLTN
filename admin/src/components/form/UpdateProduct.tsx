...
const UpdateProdcut = ({ value, vars, setVars, setAddVariants }: Props) => {
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            setVars((prevVariants) => {
                ...
                const fileWithUrl = new File([file[0]], url);
    ...
    useEffect(() => {
        if (vars[value]?.details) {
            setSizes((prevSizes) => {
                const updatedSizes = [...prevSizes];
                updatedSizes[value] = vars[value].details;
                return updatedSizes;
            });
        }
    }, []);
                    ...
                    {sizes[value]?.map((item, index) => (
                        <div key={index} className="flex items-center gap-[25px] justify-center mt-[10px]">
                            <TextField
                                ...
                                value={vars[value].details[index]?.size}
                            ...
                            <TextField
                                ...
                                value={vars[value].details[index]?.quantity}
